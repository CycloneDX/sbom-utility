...
The utility additionally prioritizes commands that help provide insight into contents of the SBOM (e.g., [license](#license), [resource](#resource) and [query](#query) commands) to search for missing or specific data requirements.  The goal of these prioritized commands is to support dat verification for many of the primary SBOM use cases as identified by the CycloneDX community (see https://cyclonedx.org/use-cases/).  Functional development has been prioritized to support those use cases that are designed toward inventory (resource identity), legal compliance (e.g., license), and security analysis (e.g., vulnerability) which are foundational to any SBOM.
